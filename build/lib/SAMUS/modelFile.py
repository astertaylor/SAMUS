...
class model:
    ...
    def run_model(self, nsrot=10, rtol=0.01, period=7.937, Cmax=1.,
        ...
        bool
            Boolean marking if the run has diverged. If True, it has diverged.
        ...
        for func in funcs:
            ...
            if type(func) is str:
                out_funcs.append(getattr(self, func))

            # if a function, add to class, then fetch
            else:
                self.add_method(func)
                out_funcs.append(getattr(self, func.__name__))
        ...
        # create output file
        self.outfile = File(
            "results/{}_{}.pvd".format(self.name,
